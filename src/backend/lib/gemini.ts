...
// The client is constructed lazily, per call, rather than once at module load.
// Reason: ES module imports are evaluated before the importing module's own
// top-level code runs, so if this were `new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY })`
// at module scope, it would resolve `process.env.GEMINI_API_KEY` before
// `dotenv.config()` in server.ts has actually populated it in local dev —
// permanently baking in an undefined key regardless of what's in .env.
// Caching by key avoids re-instantiating the client on every single request.
