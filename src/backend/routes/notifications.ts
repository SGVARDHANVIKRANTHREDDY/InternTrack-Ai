...
router.delete('/:id', authenticate, async (req: any, res) => {
  try {
    ...
    logger.error('Error deleting notification', { requestId: req.id, userId: req.userId, error: err });
    res.status(500).json({ error: 'Server error' });
  }
});

// Delete (dismiss) selected notifications in bulk
router.post('/bulk-delete', authenticate, async (req: any, res) => {
  try {
    const { ids } = req.body;
    if (!ids || !Array.isArray(ids)) {
      res.status(400).json({ error: 'Invalid payload: ids array is required' });
      return;
    }
    // Scoped to req.userId so a user can never dismiss notifications they don't own,
    // even if an id belonging to someone else were included in the array.
    ...
    logger.error('Error bulk-deleting notifications', { requestId: req.id, userId: req.userId, error: err });
    res.status(500).json({ error: 'Server error' });
  }
});
